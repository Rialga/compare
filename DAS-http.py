...
class WebRequest(object):
    """ Parent class for all TigernixERP Web request types, mostly deals with
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        TigernixERP implements token-based `CSRF protection
        ...
        * if the endpoint can be called by external parties (not from
          TigernixERP) as e.g. it is a REST API or a `webhook
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            error = {
                    'code': 200,
                    'message': "TigernixERP Server Error",
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "TigernixERP Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "TigernixERP Session Expired"
...
TigernixERP URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.tigernix.com/documentation/11.0/reference/http.html#csrf for
more details.

* if this endpoint is accessed through TigernixERP via py-QWeb form, embed a CSRF
...
class OpenERPSession(werkzeug.contrib.sessions.Session):
    ...
    def _fix_lang(self, context):
        """ TigernixERP provides languages which may not make sense and/or may not
...
def session_gc(session_store):
    if random.random() < 0.001:
        ...
        ##update 24Nov21, change to one day
        ##last_day = time.time() - 60*60*24
        #update 14Feb22, change to 8 hours
        last_day = time.time() - 60*60*8
...
class Root(object):
    """Root WSGI application for the TigernixERP Web Client.
    """
    def __init__(self):
        self._loaded = False
    ...
    def load_addons(self):
        ...
        # Note: TigernixERP versions > 11.0 does not preload addons anymore.
    ...
    def setup_session(self, httprequest):
        ...
        if sid is None:
            is_healthcheck = getattr(httprequest,'path','') == '/healthcheck' and True or False
            httprequest.session = self.session_store.new()
            if is_healthcheck:
                httprequest.session.sid = httprequest.session.sid+'_healthcheck'
        else:
    ...
    def get_response(self, httprequest, result, explicit_session):
        ...
        if not explicit_session and hasattr(response, 'set_cookie'):
            response.set_cookie(
                'session_id', httprequest.session.sid, max_age=1800, httponly=True)
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            args_db = False
            try:
                args_db = dict(httprequest.args).get('db',False) or False
            except:
                args_db = False
                pass
#            args_db = False
            ...
            if odoo.tools.config.get('prevent_crawling'):
                user_agent = str(request.httprequest.user_agent)
                black_list = ['bot', 'crawl', 'spider']

                if any(x in user_agent for x in black_list):
            ...
            with request:
                ...
                uid_check = request.session.uid
                if ((not db) or (db and not uid_check)) and args_db:
                    if isinstance(args_db,list) or isinstance(args_db,tuple):
                        db = args_db[0]
                    elif isinstance(args_db,str):
                        db = args_db
...
def db_filter(dbs, httprequest=None):
    ...
    if odoo.tools.config['dbfilter']:
        ...
        # In case --db-filter is not provided and --database is passed, TigernixERP will
    ...
    path_list = httprequest.path.split('/')
    if len(path_list) > 1 and path_list[1] in cdb_list:
        dbs = cdb_list[path_list[1]]
        httprequest.session['home_path'] = httprequest.url_root + path_list[1] + "/"
        httprequest.session['db_key'] = "/" + path_list[1]
        httprequest.session['login_path'] = httprequest.session.db_key + '/web/login'
        
    ##HANDLE PAYPAL IPN POST PAYMENT FEEDBACK(NOT REALTIME PAYMENT POSTBACK)
    if httprequest.method == 'POST':
        form    = httprequest.form
#        if form.get('test_ipn') == '1':
        custom  = form.get('custom') and json.loads(form.get('custom'))
        dbs     = custom and [custom.get('database')] or dbs
        if custom and not custom.get('database') :
            if form.get('item_name') and "Dyslexia Association of Singapore" in form.get('item_name'):
                dbs = ['DAS_PRODUCTION']
            if form.get('item_name') and "DAS International Services Ltd" in form.get('item_name'):
                dbs = ['DAI_PRODUCTION']
            if form.get('item_name') and "DAS Academy Ltd" in form.get('item_name'):
                dbs = ['DAC_PRODUCTION']
    ##HANDLE PAYPAL IPN POST PAYMENT FEEDBACK(NOT REALTIME PAYMENT POSTBACK)
    if not httprequest.session.login_path:
        httprequest.session['login_path'] = '/web/login'
    
#    UPDATE db_key AND login_path BASED ON SELECTED DB IN SESSION
    if httprequest.session.db:
        if [httprequest.session.db] in cdb_list.values():
            httprequest.session['db_key'] = "/" + list(cdb_list.keys())[list(cdb_list.values()).index([httprequest.session.db])]
            httprequest.session['login_path'] = httprequest.session.db_key + '/web/login'
#    UPDATE db_key AND login_path BASED ON SELECTED DB IN SESSION
...
class CommonController(Controller):
    ...
    def jsonrpc(self, service, method, args):
        """ Method used by client APIs to contact TigernixERP. """
