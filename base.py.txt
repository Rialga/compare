    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def _json_response(self, result=None, error=None):
        ...
        if error is not None:
            if error.get('data', {}):
                allowed_type = ['user_error', 'warning', 'access_error', 'missing_error', 'validation_error', 'except_orm', 'access_denied']
                if error['data']['exception_type'] not in allowed_type:
                    if request.env.user.id != 1:
                        if error['data'].get('debug', ''):
                            debug_id = uuid.uuid4().hex[:6]
                            error_info = {'message': error['data']['message'], 'debug': error['data']['debug']}
                            request.httprequest.session['error_log_' + str(debug_id)] = error_info
                            error['data']['debug_id'] = debug_id
                            error['data']['debug'] = "Internal Server Error"
                            error['data']['arguments'] = ["Internal Server Error"]
                            error['data']['name'] = "Internal Server Error"
        ...
        try:
            ...
            if request.env.user.id != 1:
                if error.get('data', {}):
                    if error['data'].get('debug', ''):
                        debug_id = uuid.uuid4().hex[:6]
                        request.httprequest.session['error_log_' + str(debug_id)] = error['data']['debug']
                        error['data']['debug_id'] = debug_id
                        error['data']['debug'] = "Internal Server Error"
                        error['data']['arguments'] = ["Internal Server Error"]
                        error['data']['name'] = "Internal Server Error"
...
def serialize_exception(e):
    ...
    if request.env.user.id != 1:
        if tmp.get('debug'):
            debug_id = uuid.uuid4().hex[:6]
            request.httprequest.session['error_log_' + str(debug_id)] = tmp['debug']
            tmp['debug_id'] = debug_id
            tmp['debug'] = ''
    return tmp
...
def db_filter(dbs, httprequest=None):
    ...
    cdb_list = {
        'live' : ['ILHAM_PRODUCTION'],
        'madrasah' : ['MADRASAH_PRODUCTION'],
    }
